...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-amber-lang-ir-detail.ps1

julia --project=. tests/domain_review.jl

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-amber-lang-ir-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-amber-lang-ir-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-amber_lang_ir_probe-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
